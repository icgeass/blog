...
public class AesCrypt {
     ...
     */
    public AesCrypt(final String key, final int bit) {
        this(key, bit, null);
    }
     ...
     */
    public AesCrypt(final String key, final int bit, final String iv) {
        if (bit == 256) {
    ...
    public String encrypt(final byte[] data) {
        try {
            return new String(Base64.getEncoder().encode(encryptByteArray(data)), "UTF-8");
        } catch (final Exception ex) {
            throw new RuntimeException(ex.getMessage());
        }
    }

    public byte[] encryptByteArray(final byte[] data) {
        try {
    ...
    public String decrypt(final byte[] data) {
        try {
            return new String(decryptByteArray(data), "UTF-8");
        } catch (final Exception ex) {
            throw new RuntimeException(ex.getMessage());
        }
    }

    public byte[] decryptByteArray(final byte[] data) {
        try {
